...
export const getFilter = id => ({
  ...
  promise: () => API.post(URLConf.cascadeFilter, {id, type: "eventScheduleFilter"})
});

export const getSchedule = ({eventId, date, location}) => ({
  types: [FETCH_EVENT_SCHEDULE, FETCH_EVENT_SCHEDULE_SUCCESS, FETCH_EVENT_SCHEDULE_FAILTURE],
  promise: () => API.post(URLConf.fetchEventSchedule, {eventId, date, location})
});

export const setCurrentFilter = (eventId, currentFilter) => {
  return (dispatch) => {
    dispatch({
