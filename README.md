...
一直提示用户名重复,所以无法验证
- [ ] 登录
可以登录 但返回的个人信息中缺少 身份证信息(cardId)
- [ ] 发送验证码
显示成功 但2个手机号均未收到短信
- [ ] 验证用户名
换了很多用户名 包括很复杂的 一直显示重复
- [x] ~~验证电话号码~~
OK
- [ ] 重置密码
收不到验证码暂时无法验证
...
- [ ] 个人信息(fetchMyData)
返回的都是空数据
- [ ] 详细设置信息(fetchMySettings)
接口报错  Input string was not in a correct format.
- [ ] 更新个人信息
依赖上一个接口返回的结果, 所以无法确定是否成功
- [ ] 我的约球(fetchMyPractice)
返回 请求方法名不存在，请确认方法名
- [ ] 我的比赛(fetchMyMatch)
成功返回 但数据真实性有待进一步验证 因为传过来的是空数组
